...
const Rating: React.FC<StepsProps> = ({ name, rating, img, className }) => {
  ...
  useEffect(() => {
    ...
    const step = 0.1;
    const duration = 1000;
    let incrementInterval = duration / (rating / step);
  ...
  return (
    <motion.div
      className={`absolute flex gap-x-2.5 rounded-xl bg-white/85 py-3.5 pr-5 pl-3 drop-shadow-[0_4px_30px_rgba(0,0,0,0.12)] backdrop-blur-3xl max-xl:py-2.5 max-sm:pr-6 max-sm:pl-2 ${className}`}
      ref={ref}
